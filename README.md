...
```bash
# Clone and symlink
git clone https://github.com/alexlazarian/brk.git ~/.brk
ln -s ~/.brk/brk /usr/local/bin/brk

# Or just copy the script
curl -o /usr/local/bin/brk https://raw.githubusercontent.com/alexlazarian/brk/main/brk
chmod +x /usr/local/bin/brk
```
...
The status command fetches from origin, then compares each pinned branch against `origin/<base_branch>` using `git rev-list --count`. PR info comes from `gh pr list`.
